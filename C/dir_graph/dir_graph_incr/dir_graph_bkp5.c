...
int main(int argc, char *argv[])
{
	...
	graph_run_dfs(t_graph, graph_dfs_visit_detect_DAG);
	return 0;
}
...
int
graph_run_dfs(dir_graph_t* t_graph, dir_graph_dfs_fnptr dfs_fn)
{
	int i = 0;
	int count = 0;
	int ret = 0;
	...
	for(i = 1; i <= t_graph->num_v; i++) {
		...
		if (v->num_dependent == 0) {
			ret = dfs_fn(t_graph, v);
		}

		if (ret != 0) {
			printf("\n This graph is not DAG, there exists cycle, for vertex [%d]\n", i);
			break;
		}
	}
	...
	printf("\n Visisted vertexs = %d, total vertexs = %d\n", count, t_graph->num_v);
}

int
graph_dfs_visit(dir_graph_t* t_graph, graph_V_t* v)
{
	...
	v->visited = 1;
	while(next_ele != DEP_CHAIN_SENTINAL) {
		if (!t_graph->v_set[next_ele]->visited) {
			graph_dfs_visit(t_graph, t_graph->v_set[next_ele]);
		}
		next_ele = DEP_ELE_GET_LAST(v->adj[next_ele]);
	}
	return 0;
...
int
graph_dfs_visit_detect_DAG(dir_graph_t* t_graph, graph_V_t* v)
{
	...
	while(next_ele != DEP_CHAIN_SENTINAL) {
		if (!t_graph->v_set[next_ele]->visited) {
			ret = graph_dfs_visit_detect_DAG(t_graph, t_graph->v_set[next_ele]);
		}
		if (ret != 0) {
			break;
		}
